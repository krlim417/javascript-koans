...
describe("About Applying What We Have Learnt", function() {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function () {
      ...
      _(products).filter(function(product) {
        var doesNotContainMushroom = _(product.ingredients).any(function(ingredient) {
          return !ingredient.includes('mushroom');
        });
        if (doesNotContainMushroom && product.containsNuts === false) {
          return product;
        }
      });
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    ...
    var sum = _(_.range(1, 1000)).chain().reduce(function(total, num) {
      if (num % 3 === 0 || num % 5 === 0) {
        return total + num;
      } else {
        return total;
